...
We define 
$$g(G, n) = \min \left\{ s \in \Z_{\geq 0} \Big| \forall_{g_1, \ldots, g_s \in G} \exists_{I \subseteq [s]} \text{ s.t. } |I| = n \, \text{ and } \sum_{i \in I} g_i = 0 \right\}\, . $$
...
Indeed, take $g_{0, i} = (0, 0)$, $g_{(1, i)} = (1, 0)$, $g_{(2, i)} = (0, 1) $ and $g_{3, i} = (1, 1)$ for $i=1, \ldots, p-1$, and there is no non-trivial subset that results in a zero sum:
if $\sum_{j=0}^3 a_j g_{(j, 1)} = (0, 0) $ for $a_0, \ldots, a_3 \in \{0, \ldots, p-1\}$ with , then 
