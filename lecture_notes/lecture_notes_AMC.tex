...
\begin{prop}[Frankl - Wilson generalisation]
Let $E$ be a finite set, $p$ a prime number, $\lambda$ a positive integer and $L$ a collection of elements in $\F_p$.
Assume that $\FF$ is $L$-Fischer and $|F| = \lambda $ for each $F \in \FF$.
...
We can therefore use \cref{prop:frankl_wilson_fixed_size}, which gives us that $m \leq \binom{n}{|L|}$.
...
Thus according to \cref{prop:RCW}, we have that $m \leq \binom{n}{|L|}$, this shows that the presented colouring is $(\binom{n}{p-1}, \binom{n}{p-1})$-Ramsey.


\end{proof}

\subsection{Chromatic number of the real space}
...
In this sectio we compute the chromatic number of this 
