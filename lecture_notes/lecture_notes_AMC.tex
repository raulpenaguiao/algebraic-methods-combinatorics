...
Define $\vy_i = (\vx_i, 1)\in \R^{d+1}$ for $i = 0, \ldots, m$, define $\vv_i = \vec{e}_i \in\R^{r-1}$ for $i = 1, \ldots, r-1$, and define $\vv_r = - \sum_{i=1}^{r-1} \vec{e}_i$.
...
%Recall that $\F_q $ is the finite field of size $q$.
...
Every graph $G$ arising from adding an edge to a $4$-regular graph contains a $3$ regular graph
\end{thm}

This result gets us very close to an open conjecture in graph theory.

\begin{conj}[Berge Saun conjecture]
Every $4$-regular graph contains a $3$-regular graph.
\end{conj}


\begin{proof}[Proof of \cref{thm:3reggraphs}]
For each vertex $v\in V(G)$ consider the polynomial in $\F_3[\vx_e | e\in E(G) ]$ defined by
